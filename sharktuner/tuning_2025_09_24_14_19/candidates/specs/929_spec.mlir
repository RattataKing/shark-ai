module attributes {iree_codegen.tuning_spec_with_default_entrypoint, transform.with_named_sequence} {
  ...
  transform.named_sequence @match_main_dispatch_0_matmul_8192x8192x8192_f16xf16xf32(%arg0: !transform.any_op {transform.readonly}) -> (!transform.any_op, !transform.any_param) {
    ...
    ^bb0(%arg1: tensor<8192x8192xf16>, %arg2: tensor<8192x8192xf16>, %arg3: tensor<8192x8192xf32>):
      %1 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%arg1, %arg2 : tensor<8192x8192xf16>, tensor<8192x8192xf16>) outs(%arg3 : tensor<8192x8192xf32>) {
