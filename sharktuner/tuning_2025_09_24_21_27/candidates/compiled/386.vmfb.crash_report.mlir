...
module {
  ...
  hal.executable private @main_dispatch_0 {
    hal.executable.variant public @rocm_hsaco_fb target(#executable_target_rocm_hsaco_fb) {
      hal.executable.export public @main_dispatch_0_matmul_64x640x2048_f16xf16xf32 ordinal(0) layout(#pipeline_layout) count(%arg0: !hal.device) -> (index, index, index) {
      ...
      builtin.module {
        func.func @main_dispatch_0_matmul_64x640x2048_f16xf16xf32() {
          ...
          %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c0) flags("ReadOnly|Indirect") {iree_gpu.use_rocdl_buffer_instructions} : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x2048xf16>>
          %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) flags("ReadOnly|Indirect") {iree_gpu.use_rocdl_buffer_instructions} : !iree_tensor_ext.dispatch.tensor<readonly:tensor<640x2048xf16>>
          ...
          %3 = iree_tensor_ext.dispatch.tensor.load %0, offsets = [0, 0], sizes = [64, 2048], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x2048xf16>> -> tensor<64x2048xf16>
          %4 = iree_tensor_ext.dispatch.tensor.load %1, offsets = [0, 0], sizes = [640, 2048], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<640x2048xf16>> -> tensor<640x2048xf16>
          %5 = tensor.empty() : tensor<64x640xf32>
          %6 = linalg.fill ins(%cst : f32) outs(%5 : tensor<64x640xf32>) -> tensor<64x640xf32>
          %7 = linalg.generic {indexing_maps = [#map, #map1, #map2], iterator_types = ["parallel", "parallel", "reduction"]} ins(%3, %4 : tensor<64x2048xf16>, tensor<640x2048xf16>) outs(%6 : tensor<64x640xf32>) {
